...
from cogs.utils.db import ensure_wallet, update_wallet, lock_wallet, add_transaction
...
async def _extract_lichess(session: aiohttp.ClientSession, url: str):
    ...
    # Download GIF with hidden players/ratings
    gif_url = (
...
# Registry — extensible for future sites
SITE_EXTRACTORS = {
...
# Sites recognised but not yet implemented
...
class GTE(commands.Cog):
    ...
    async def gte(self, ctx: commands.Context, lichess_link: str, award: int, duration: str = "60s"):
        ...
        # Deduct award from starter's wallet
        ...
        self.games[ctx.channel.id] = {
            ...
            "variant": variant,
            "tc": tc_str,
            "guesses": {},  # user_id -> (white_guess, black_guess)
            "phase": "guessing",
            "duration": seconds,
        }
