...
from cogs.utils.db import (
    ensure_wallet, update_wallet, add_transaction, lock_wallet,
...
class Offers(commands.Cog):
    ...
    async def offer(self, ctx, *, args: str = ""):
        ...
        # Worst-case liability for a single max-stake take:
    ...
    async def closeoffer(self, ctx, offer_id: int, outcome: str):
        ...
        async with self.pool_conn.acquire() as conn:
            async with conn.transaction():
                ...
                payout_lines = []
                if outcome == "win":
                    # Players win: each take receives stake * odds
                    for t in takes:
                    ...
                    # Host gets back the unused portion of the pool
                    ...
                    # Host wins: host receives all stakes + their full pool back
