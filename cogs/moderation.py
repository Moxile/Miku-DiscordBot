...
def parse_duration(text):
    """ Translates a duration into timedelta """
    match = re.fullmatch(r"(\d+)([smhd])", text.lower()) # matches number followed by s, m, h or d
    if not match:
        return None
    value, unit = int(match.group(1)), match.group(2) # group 1 extracts (\d+) and group 2 extracts ([smhd])
...
class Moderation(commands.Cog):
    ...
    async def check_target(self, ctx: commands.Context, member: discord.Member) -> bool:
        """Checks if the target member can be moderated by the command and the bot."""
