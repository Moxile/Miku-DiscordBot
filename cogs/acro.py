...
class Acro(commands.Cog):
    ...
    async def acro(self, ctx: commands.Context, bet: str = None):
        ...
        self.games[ctx.channel.id] = {
            ...
            "bet": bet,
            "phase": "guessing",
            "guesses": {},            # user_id -> guess string
            "votes": {},              # voter_id -> submission index (0-based)
            "participants": set(),    # user_ids who paid
            "submission_order": [],   # user_ids in display order
        }
    ...
    async def _resolve(self, channel: discord.TextChannel, game: dict):
        ...
        # Apply -1 penalty for players who didn't vote
    ...
    async def _handle_guess(self, message: discord.Message, game: dict):
        ...
        # Check if it matches the letter pattern
        ...
        # Valid guess — delete the message first
        try:
        ...
        # If already a participant, just update the guess (no double charge)
        if uid in game["participants"]:
            game["guesses"][uid] = message.content.strip()
            return

        # New participant — charge the bet if applicable
        if game["bet"]:
    ...
    async def _handle_vote(self, message: discord.Message, game: dict):
        ...
        game["votes"][voter_id] = choice - 1  # 0-based index
